...
export class SolanaAddressProvider implements Provider {
  ...
  async get(runtime: IAgentRuntime, message: Memory, state: State): Promise<string | null> {
        ...
        elizaLogger.log("Retrieving onchain address from blockchain data runtime manager...");
...
function concatenateMemories(memories: Memory[]): string {
  const prefix = "A list of relevant on-chain txs content: ";
